...
declare -a tests=(
"s21_grep.c s21_grep.h Makefile -e for VAR"
"s21_grep.c -e for VAR"
"s21_grep.c s21_grep.h Makefile -e for -e int VAR"
"s21_grep.c -e for -e int VAR"
"s21_grep.c -e regex -e print VAR"
"s21_grep.c Makefile -e while -e void VAR"
"VAR no_file.txt"
)
...
testing()
{
    ...
    valgrind --leak-check=full ./s21_grep $t > test_s21_grep.log
    leak=$(grep -A100000 leaks test_s21_grep.log)
    (( COUNTER++ ))
    if [[ $leak == *"0 leaks for 0 total leaked bytes"* ]]
    then
      (( SUCCESS++ ))
        echo -e "\033[0m$COUNTER grep $t"
    else
      (( FAIL++ ))
        echo -e "\033[0m$COUNTER grep $t"
#        echo "$leak"
